...
void measure_laser()
{
    ...
    for (int i = 0; i < TotalFile; i++)
    {
		...
		sprintf(FileNamefront,"%d",440+i);
		sprintf(FileName,"%s.csv",FileNamefront);
		sprintf(GraphTitle,"%d;wavelength(nm);intensity",440+i);
...
//		tf[i]->Draw("same");
    ...
    d->SetTitle("440~465nm5ms7.8%;wavelength(nm);sigma(nm)");
