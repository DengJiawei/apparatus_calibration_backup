...
void measure_laser()
{
    ...
    for (int i = 0; i < TotalFile; i++)
    {
		...
		sprintf(FileNamefront,"%d",410+i);
		sprintf(FileName,"%s.csv",FileNamefront);
		sprintf(GraphTitle,"%d;wavelength(nm);intensity",410+i);
...
//		tf[i]->Draw("same");
    ...
    d->SetTitle("410~440nm5ms7.9%;wavelength(nm);sigma(nm)");
